...
root_name = '%ssimple_%scomplex_%sreg_%sweight_%sepoch_%srate_%smomentum' % (args.projectionNetNum, args.complexNetNum,
                                                               args.regularizationNum, args.projection_weight,
                                                               args.epoch, args.lr, args.momentum)
    ...
    if shifted_dataset is not None:
        ...
        with torch.no_grad():
            for batch_idx, (test_data, shifted_test_data) in enumerate(zip(testloader, testloader_shifted)):
                ...
                distribution_mismatch_loss += args.projection_weight * projection_criterion(test_outputs,
                                                                                            shifted_test_outputs,
                                                                                            batch_idx).item()
...
for aug in range(round, len(net_list)):
    ...
    for epoch in range(start_epoch, end_epoch):
        optimizer = optim.SGD(net.parameters(), lr=get_lr(epoch, lr), momentum=args.momentum, weight_decay=0.001) # 0.0005, momentum 0.9, lr 0.001
        if reg_list[aug]:
        ...
        if no_improve:
            ...
            decrease_rate_count = 0
            stop_count = 0
        if decrease_rate_count >= 8:
            lr = lr / 3
            decrease_rate_count = 0
            stop_count += 1
            net.load_state_dict(best_model_state)
            print("new learning rate %s" % lr)
        if stop_count > 3:
            break
